...
def _set_unique_constraints(tx, node):
    """
    That creates and runs queries enforcing each node to have a unique ID
    
    Args:
        tx (neo4j.Transaction): The Neo4j transaction object.
        node (str): The name of the node type to set the unique constraint on.
...
def load_hospital_graph_from_csv() -> None:
    ...
    This function sets up uniqueness constraints on nodes and then loads the data
    from the CSV files into Neo4j.
    ...
    with driver.session(database="neo4j") as session:
        for node in NODES: 
            session.execute_write(_set_unique_constraints, node)
    
    LOGGER.info("Load hospital nodes...")
    with driver.session(database="neo4j") as session:
        query = f"""
